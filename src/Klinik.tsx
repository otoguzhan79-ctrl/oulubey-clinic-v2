...
const Klinik = () => {
  ...
  useEffect(() => {
    // Videoyu biraz daha hızlı yükletelim
    setLoadVideo(false);
  ...
  return (
    <div className="relative min-h-screen font-sans antialiased text-slate-900 overflow-x-hidden">
      ...
      {/* 1. EN ARKA KATMAN: Sabit Renk */}
      <div className="fixed inset-0 bg-slate-100 -z-30" />
      
      {/* 2. VİDEO KATMANI: Renkten bir üstte */}
      {loadVideo && (
        <video 
      ...
      {/* 3. SİS PERDESİ: Videonun üstünde, yazının altında */}
      ...
      <main className="relative pt-32 pb-20 px-6 max-w-7xl mx-auto text-center z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-24">
          {currentData.services.map((item, index) => (
            <div key={index} className="bg-white/80 backdrop-blur-md border border-white/50 p-8 rounded-[2.5rem] shadow-xl hover:-translate-y-2 transition-all duration-500 group">
      ...
      {modalInfo.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-sm bg-black/20">
          <div className="bg-white rounded-[2rem] p-10 max-w-md w-full relative animate-fade-in shadow-2xl">
            ...
            <button onClick={() => setModalInfo({ ...modalInfo, isOpen: false })} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold hover:bg-slate-900 transition-all">Anladım</button>
          </div>
        </div>
      )}
    </div>
